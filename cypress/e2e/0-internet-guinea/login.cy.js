...
describe('The Internet Guinea Pig Website', () => {
  ...
  it('Login with valid credentials', function () {
    cy.get('#username').should('have.value', '').type('tomsmith').should('have.value', 'tomsmith');
    cy.get('#password').should('have.value', '').type('SuperSecretPassword!');
  ...
  it('Login with valid credentials - data fixture example', function () {
    ...
    cy.get('[type="submit"]').click();
    cy.get('#flash').should('contain', this.userData.message);
  });
